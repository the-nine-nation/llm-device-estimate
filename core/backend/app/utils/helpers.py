...
def recommend_gpus(memory_requirement_gb: float, max_count: int = 8, 
    ...
    for gpu_name, specs in GPU_SPECS.items():
        ...
        if gpu_memory >= memory_requirement_gb:
            gpu_info = GPUInfo(
                ...
                fp16_tflops=specs.get("fp16_tflops")
            )
        ...
        for card_count in [2, 4, 8]:
            ...
            if total_memory >= memory_requirement_gb and card_count <= 8:
                ...
                total_fp16_tflops = specs.get("fp16_tflops", 0) * card_count if specs.get("fp16_tflops") else None
    ...
    def sort_key(gpu: GPUInfo) -> float:
        ...
        # 算力效率评分
        if gpu.fp16_tflops:
            compute_efficiency = gpu.fp16_tflops / gpu.memory_gb
        else:
        ...
        if use_case == "inference":
            ...
            if "x" not in gpu.name:  # 单卡优先
                return memory_utilization * 0.7 + compute_efficiency * 0.3 + 0.2
            else:
                return memory_utilization * 0.7 + compute_efficiency * 0.3
        else:
            # 训练更看重总显存和算力效率
            return memory_utilization * 0.5 + compute_efficiency * 0.5
...
def calculate_compute_power(gpu_count: int, gpu_name: str) -> Optional[float]:
    """
    计算总算力
    ...
    Returns:
        总算力（TFLOPS），如果没有算力信息则返回None
    """
    if gpu_name not in GPU_SPECS:
        return None
    
    gpu_specs = GPU_SPECS[gpu_name]
    if "fp16_tflops" not in gpu_specs:
        return None
    
    # 计算总算力
    total_compute_power = gpu_count * gpu_specs["fp16_tflops"]
...
def optimize_batch_size(model_params: int, sequence_length: int, 
    ...
    Args:
        ...
        target_memory_gb: 目标显存限制（GB）
        
    Returns:
        推荐的批次大小
    """
    # 粗略估算：每个样本的激活值显存约为 seq_len * hidden_size * 4 bytes
    # 这里使用经验公式
